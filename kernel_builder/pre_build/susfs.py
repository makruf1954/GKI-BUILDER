...
class SUSFSPatcher:
    ...
    def apply(self) -> None:
        ...
        SUSFS_VERSION: str = re.search(
            r"v\d+\.\d+\.\d+", (SUSFS / "include" / "linux" / "susfs.h").read_text()
        ).group()  # pyright: ignore[reportOptionalMemberAccess]
        NEXT_SUSFS_FIX: Path = (
            WORKSPACE / "wild_patches" / "next" / "susfs_fix_patches" / SUSFS_VERSION
        )
        ...
        if self.ksu_variant == "NEXT":
            log("Manually applying SUSFS patch for KernelSU-Next")
            KSUN_PATH: Path = WORKSPACE / "KernelSU-Next"
            apply_patch(KSU_SUSFS, check=False, cwd=KSUN_PATH)
            self._apply_patch_folder(NEXT_SUSFS_FIX, KSUN_PATH)
